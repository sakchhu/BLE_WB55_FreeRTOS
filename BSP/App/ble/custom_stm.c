...
/* Private typedef -----------------------------------------------------------*/
typedef struct {
    uint16_t nus_service_handle; /**< Nordic_UART_Service handle */
    uint16_t nus_rx_handle;  /**< RX_Characteristic handle */
    uint16_t nus_tx_handle;  /**< TX_Characteristic handle */
    uint16_t nus_blob_rx_handle;
    uint16_t nus_blob_tx_handle;
} profile_context_t;
 ...
 */
static profile_context_t gs_gatt_profile_ctx;
...
#define COPY_BLOB_RX_CHARACTERISTIC_UUID(uuid_struct) COPY_UUID_128(uuid_struct, 0x6e, 0x40, 0x00, 0x12, 0xb5, 0xa3, 0xf3, 0x93, 0xe0, 0xa9, 0xe5, 0x0e, 0x24, 0xdc, 0xca, 0x9e)
#define COPY_BLOB_TX_CHARACTERISTIC_UUID(uuid_struct) COPY_UUID_128(uuid_struct, 0x6e, 0x40, 0x00, 0x13, 0xb5, 0xa3, 0xf3, 0x93, 0xe0, 0xa9, 0xe5, 0x0e, 0x24, 0xdc, 0xca, 0x9e)
...
static SVCCTL_EvtAckStatus_t Custom_STM_Event_Handler(void *Event) {
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
        ...
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
            ...
            if (attribute_modified->Attr_Handle == (gs_gatt_profile_ctx.nus_tx_handle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
            {
            ...
            else if (attribute_modified->Attr_Handle == (gs_gatt_profile_ctx.nus_blob_tx_handle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
            {
                return_value = SVCCTL_EvtAckFlowEnable;
                ...
                /* Disabled Notification management */
                case (!(COMSVC_Notification)):
                    Notification.Custom_Evt_Opcode = CUSTOM_STM_BLOB_TX_NOTIFY_DISABLED_EVT;
                    Custom_STM_App_Notification(&Notification);
                ...
                /* Enabled Notification management */
                case COMSVC_Notification:
                    Notification.Custom_Evt_Opcode = CUSTOM_STM_BLOB_TX_NOTIFY_ENABLED_EVT;
                    Custom_STM_App_Notification(&Notification);
                break;
            ...
            else if (attribute_modified->Attr_Handle == (gs_gatt_profile_ctx.nus_rx_handle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
            {
                return_value = SVCCTL_EvtAckFlowEnable;
            ...
            else if (attribute_modified->Attr_Handle == (gs_gatt_profile_ctx.nus_blob_rx_handle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
            {
                return_value = SVCCTL_EvtAckFlowEnable;
                Notification.Custom_Evt_Opcode = CUSTOM_STM_BLOB_RX_WRITE_NO_RESP_EVT;
                Notification.DataTransfered.Length = attribute_modified->Attr_Data_Length;
                Notification.DataTransfered.pPayload = attribute_modified->Attr_Data;
        ...
        case ACI_GATT_WRITE_PERMIT_REQ_VSEVT_CODE:
            ...
            if (write_perm_req->Attribute_Handle == (gs_gatt_profile_ctx.nus_rx_handle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET)) {
                return_value = SVCCTL_EvtAckFlowEnable;
...
void SVCCTL_InitCustomSvc(void) {
    ...
    max_attr_record += 5; /* BLOB_TX (3) + BLOB_RX (2)*/
    ...
    ret = aci_gatt_add_char(gs_gatt_profile_ctx.nus_service_handle,
                            UUID_TYPE_128, &uuid,
                            SizeRx,
    ...
    ret = aci_gatt_add_char(gs_gatt_profile_ctx.nus_service_handle,
                            ...
                            0x10,
                            CHAR_VALUE_LEN_VARIABLE,
                            &(gs_gatt_profile_ctx.nus_blob_rx_handle));
    if (ret != BLE_STATUS_SUCCESS) {
        printf("  Fail   : aci_gatt_add_char command   : BLOB_RX, error code: 0x%x \n\r", ret);
    } else {
        printf("  Success: aci_gatt_add_char command   : BLOB_RX , handle = 0x%04x \n\r", gs_gatt_profile_ctx.nus_blob_rx_handle);
    }

    /**
     *  BLOB_TX_Characteristic
     */
    COPY_BLOB_TX_CHARACTERISTIC_UUID(uuid.Char_UUID_128);
    ret = aci_gatt_add_char(gs_gatt_profile_ctx.nus_service_handle,
                            UUID_TYPE_128, &uuid,
                            SizeTx,
                            CHAR_PROP_NOTIFY,
                            ...
                            0x10,
                            CHAR_VALUE_LEN_VARIABLE,
                            &(gs_gatt_profile_ctx.nus_blob_tx_handle));
    if (ret != BLE_STATUS_SUCCESS) {
        printf("  Fail   : aci_gatt_add_char command   : BLOB_TX, error code: 0x%x \n\r", ret);
    } else {
        printf("  Success: aci_gatt_add_char command   : BLOB_TX , handle = 0x%04x \n\r", gs_gatt_profile_ctx.nus_blob_tx_handle);
    }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload) {
    ...
    {
    case CUSTOM_STM_RX:
        ret = aci_gatt_update_char_value(gs_gatt_profile_ctx.nus_service_handle,
                                         gs_gatt_profile_ctx.nus_rx_handle,
                                         0,      /* charValOffset */
                                         SizeRx, /* charValueLen */
                                         (uint8_t *)pPayload);
    ...
    case CUSTOM_STM_BLOB_RX:
        ret = aci_gatt_update_char_value(gs_gatt_profile_ctx.nus_service_handle,
                                         ...
                                         (uint8_t *)pPayload);
        if (ret != BLE_STATUS_SUCCESS) {
            printf("  Fail   : aci_gatt_update_char_value BLOB_RX command, result : 0x%x \n\r", ret);
        } else {
            printf("  Success: aci_gatt_update_char_value BLOB_RX command\n\r");
        }
    break;

    case CUSTOM_STM_BLOB_TX:
        ret = aci_gatt_update_char_value(gs_gatt_profile_ctx.nus_service_handle,
                                         gs_gatt_profile_ctx.nus_blob_tx_handle,
                                         0,      /* charValOffset */
                                         SizeTx, /* charValueLen */
                                         (uint8_t *)pPayload);
        if (ret != BLE_STATUS_SUCCESS) {
            printf("  Fail   : aci_gatt_update_char_value BLOB_TX command, result : 0x%x \n\r", ret);
        } else {
            printf("  Success: aci_gatt_update_char_value BLOB_TX command\n\r");
        }
    break;
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload) {
    ...
    {
    case CUSTOM_STM_RX:
        ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, gs_gatt_profile_ctx.nus_service_handle, gs_gatt_profile_ctx.nus_rx_handle, SizeRx, pPayload);
    ...
    case CUSTOM_STM_TX:
        ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, gs_gatt_profile_ctx.nus_service_handle, gs_gatt_profile_ctx.nus_tx_handle, SizeTx, pPayload);
    ...
    case CUSTOM_STM_BLOB_RX:
        ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, gs_gatt_profile_ctx.nus_service_handle, gs_gatt_profile_ctx.nus_blob_rx_handle, SizeRx, pPayload);
    ...
    case CUSTOM_STM_BLOB_TX:
        ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, gs_gatt_profile_ctx.nus_service_handle, gs_gatt_profile_ctx.nus_blob_tx_handle, SizeTx, pPayload);
